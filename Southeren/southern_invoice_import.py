...
engine.SUPPLIER = "Southern"

STOP_RE = re.compile(
    r"^(?:Deliver to:|Total Net Amount|Carriage Net|Total VAT Amount|"
    r"Invoice Total|Bank details|These goods remain)",
    re.I,
)
...
def _parse_southern_rows(text):
    ...
    for line in lines:
        if line.startswith("Quantity Code Details"):
            in_items = True
            continue
        if not in_items:
            continue
...
def parse_southern_invoice(pdf_path):
    ...
    invoice_match = re.search(r"\bInvoice/Cr\s+No:\s*(\d+)\b", text, re.I)
    ...
    return {
        ...
        "items": _parse_southern_rows(text),
        "error": None,
    }


def _ensure_southern_workbook(workbook_path):
    ...
    if not os.path.isfile(template_path):
        raise FileNotFoundError(
            f"Southern workbook not found and template is missing: {workbook_path}"
        )

    os.makedirs(os.path.dirname(os.path.abspath(workbook_path)), exist_ok=True)
    shutil.copy2(template_path, workbook_path)
    print(f"Created Southern workbook: {workbook_path}")


engine.parse_invoice = parse_southern_invoice


if __name__ == "__main__":
    if len(os.sys.argv) >= 3:
        _ensure_southern_workbook(os.sys.argv[2])
    engine.main()
